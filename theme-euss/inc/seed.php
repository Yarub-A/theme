    ...
    foreach ( $menu_structure as $location => $items ) {
        ...
        if ( $existing ) {
            foreach ( $existing as $menu_item ) {
                $type      = $menu_item->type ?? 'post_type';
                $object    = $menu_item->object ?? 'page';
                $object_id = (int) ( $menu_item->object_id ?? 0 );
                $parent_id = (int) $menu_item->menu_item_parent;

                $map_key_parts = [ $type, $object, $object_id, $parent_id ];

                if ( 'custom' === $type ) {
                    $url            = isset( $menu_item->url ) ? (string) $menu_item->url : '';
                    $map_key_parts[] = md5( $url );
                }

                $existing_map[ implode( ':', $map_key_parts ) ] = (int) $menu_item->ID;
            }
        }

        theme_euss_sync_menu_branch( $items, $pages, $posts, $menu_id, $lang, 0, $existing_map, $used_items );
        ...
        if ( $existing ) {
            foreach ( $existing as $menu_item ) {
                ...
                if ( 'nav_menu_item' === $menu_post_type && is_string( $seed_meta ) && str_starts_with( $seed_meta, 'menu-' ) ) {
...
function theme_euss_sync_menu_branch( array $items, array $pages, array $posts, $menu_id, $lang, $parent_item_id, array &$existing_map, array &$used_items ) {
    foreach ( $items as $item ) {
        ...
        if ( isset( $existing_map[ $map_key ] ) ) {
            $menu_item_id = (int) $existing_map[ $map_key ];
        } else {
            $menu_data = [
            ...
            $existing_map[ $map_key ] = $menu_item_id;
        }

        if ( $menu_item_id > 0 ) {
            $seed_menu_key = sanitize_key( 'menu-' . $lang . '-' . $map_key );
            update_post_meta( $menu_item_id, '_theme_euss_seed_menu', $seed_menu_key );
        ...
        if ( ! empty( $item['children'] ) && $menu_item_id > 0 ) {
            theme_euss_sync_menu_branch( $item['children'], $pages, $posts, $menu_id, $lang, $menu_item_id, $existing_map, $used_items );
        }
    }
}
