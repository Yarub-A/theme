...
function theme_euss_language_switcher( $context = 'desktop' ) {
    ...
    $resolve_page_url = static function ( $slug ) {
        ...
        if ( $candidate instanceof WP_Post ) {
            return get_permalink( $candidate );
        }
    ...
    $fallback_languages = [
        'ar' => [
            ...
            'code'  => 'AR',
            'url'   => $resolve_page_url( 'home-ar' ),
        ],
        'en' => [
            'label' => __( 'English', 'theme-euss' ),
            'code'  => 'EN',
            'url'   => $resolve_page_url( 'home-en' ),
        ],
    ];

    if ( empty( $fallback_languages['en']['url'] ) ) {
        $fallback_languages['en']['url'] = $default_front_url;
    }

    if ( empty( $fallback_languages['ar']['url'] ) ) {
        $fallback_languages['ar']['url'] = home_url( '/ar/' );
    }
